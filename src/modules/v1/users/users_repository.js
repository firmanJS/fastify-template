...
const Repo = require('../../../models/postgre/core_postgres')
const {
...
const condition = (builder, where, search) => {
  if (search) {
    builder.where(where).whereILike('username', `%${search}%`).andWhere('deleted_at', null)
...
exports.create = async (req, payload) => {
  ...
  try {
    const result = await Repo.insertTrx({
      table: TABLES.USERS, payload, column: COLUMN[0], trx
    })
    await trx.commit()
...
exports.getByParam = async (req, options, column = COLUMN) => {
  try {
    ...
    const [result] = await Repo.fetchByParam({ table: TABLES.USERS, where: options.where, column })
    if (result) {
...
exports.update = async (req, options) => {
  try {
    ...
    const result = await Repo.updated({ table: TABLES.USERS, column: COLUMN[0], ...options })
    if (result) {
